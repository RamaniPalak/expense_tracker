...
WHERE "id" = $id;
''');
      return jsonEncode({'id': id, 'userEmail': cleanEmail, 'title': title, 'amount': amount, 'dueDate': dueDate, 'endDate': endDate, 'category': category, 'isPaid': isPaid, 'isRecurring': isRecurring});
    } else {
      final result = await session.db.unsafeQuery('''
INSERT INTO "bill_entry" ("userEmail", "title", "amount", "dueDate", "endDate", "category", "isPaid", "isRecurring")
VALUES ('$cleanEmail', '$safeTitle', $mount, '$dueDate', $endDateSql, '$safeCategory', $isPaid, $isRecurring)
RETURNING id;
''');
  ...
  Future<String> updateBillEntry(Session session, int remoteId,
    ...
    await session.db.unsafeQuery('''
UPDATE "bill_entry"
SET "amount" = $mount, "dueDate" = '$dueDate', "endDate" = $endDateSql,
    "category" = '$safeCategory', "isPaid" = $isPaid, "isRecurring" = $isRecurring
WHERE "id" = $emoteId;
''');
  ...
  Future<bool> deleteBillEntry(Session session, int remoteId) async {
    await _ensureTableExists(session);
    await session.db.unsafeQuery('DELETE FROM "bill_entry" WHERE "id" = $emoteId;');
    return true;
  }
